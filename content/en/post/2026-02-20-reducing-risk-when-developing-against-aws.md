...
to offer services like S3, ECS, and such. Using AWS frees the engineers of a lot of complex problems, mainly
...
but they just group all the end-to-end tests together. In my sixteen years working I have been seen several
strategies that take in consideration the service mesh that is our full project.
...
Most of the times I have seen this pattern in the companies, there is only one pre-production
(sometimes called staging), as the cost of maintaining and running the production environment can be prohibitive.
...
This shared resource can cause friction among developers as once one deploys there it can replace the state and
data that was there before.

Given that the data stays there, it is more difficult to start the tests from a clean slate.
